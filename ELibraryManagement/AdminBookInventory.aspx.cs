...
namespace ELibraryManagement
{
	public partial class AdminBookInventory : System.Web.UI.Page
	{
		...
		protected void Button2_Click(object sender, EventArgs e)
		{
			...
			{
				Response.Write("<script>alert('Book with same ID already exists. Try another one')</script>");
			}
			else
			{
				addNewBook();
		...
		bool checkIfBookExists()
		{
			try
			{
				...
				SqlCommand cmd = new SqlCommand("SELECT * FROM book_master_table WHERE book_id= '" + TextBox1.Text.Trim() + "' OR book_name= '" + TextBox10.Text.Trim() + "';", con);
		...
		void addNewBook()
		{
			try
			{
				...
				string filepath = "~/Book_Inventory/GeneralCover.png";
				...
				SqlCommand cmd = new SqlCommand("INSERT INTO book_master_table(book_id,book_name,genre,author_name,publisher_name,publisher_date,language,edition,book_cost,book_description,actual_stock,current_stock,book_img_link) values(@book_id,@book_name,@genre,@author_name,@publisher_name,@publisher_date,@language,@edition,@book_cost,@book_description,@actual_stock,@current_stock,@book_img_link)", con);

				cmd.Parameters.AddWithValue("@book_id", TextBox1.Text.Trim());
				cmd.Parameters.AddWithValue("@book_name", TextBox10.Text.Trim());
		...
		void getBookByID()
		{
			try
			{
				...
				if (dt.Rows.Count >= 1)
				{
					TextBox10.Text = dt.Rows[0]["book_name"].ToString();
